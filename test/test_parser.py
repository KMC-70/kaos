...
class TestEphemerisParser(KaosTestCase):
    ...
    def test_db_add_correct_orbit_point_data(self):
	...
	query = orbit_segment.query.all()

	for q, seg_start in zip(query, seg_times[:-1]):
	    self.assertTrue(q.start_time == seg_start)

	for q, seg_end in zip(query, seg_times[1:]):
	    self.assertTrue(q.end_time == seg_end)
